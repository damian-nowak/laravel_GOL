...
//        $checkedPrevGameField = $this->healthCheck($this->prevGameField);
        $checkedPrevGameField = $this->prevGameField;
...
//                echo "suma sasiadow row:$row,col: $col => " . $sumOfNeighbours . "<br>";

                switch ($this->gameField[$row][$col]['status']) {
                    case 'new':
                        if ($sumOfNeighbours >= 1 && $greenNeighbours > $blueNeighbours) {
                            $this->gameField[$row][$col]['blue'] = 0;
                            $this->gameField[$row][$col]['green'] = 1;
                            $this->gameField[$row][$col]['status'] = 'alive';
                        }
                        if ($sumOfNeighbours >= 1 && $greenNeighbours <= $blueNeighbours) {
                            $this->gameField[$row][$col]['blue'] = 1;
                            $this->gameField[$row][$col]['green'] = 0;
                            $this->gameField[$row][$col]['status'] = 'alive';
                        }
                        break;
                    case 'alive':
                        if ($sumOfNeighbours > 3) {
                            $this->gameField[$row][$col]['status'] = 'dead';
                            $this->gameField[$row][$col]['blue'] = $this->gameField[$row][$col]['blue'];
                            $this->gameField[$row][$col]['green'] = $this->gameField[$row][$col]['green'];
...
//                }
//            }
//            if ($this->gameField[$row][$col]['status'] == 'dead') {
//                if ($sumOfNeighbours == 3) {
//                    $this->gameField[$row][$col]['status'] = 'alive';
//                    $this->gameField[$row][$col]['blue'] = $this->gameField[$row][$col]['blue'];
//                    $this->gameField[$row][$col]['green'] = $this->gameField[$row][$col]['green'];
    ...
    private function healthCheck($array) {
        ...
        for ($row = 0; $row < sizeof($array); $row++) {
            for ($col = 0; $col < sizeof($array); $col++) {
                ...
                // test - TO REMOVE
//                echo "row: $row col: $col sasiedzi: $sumOfNeighbours" . "<br>";
                //
                // setting proper status
